...
const Inventory = memo(({ disabled }: { disabled?: boolean }): ReactElement => {
  ...
  const [weaponItem, setWeaponItem] = useState<
    Partial<Weapon<'Leve' | 'Pesada'>>
  >({});
  const [armorItem, setArmorItem] = useState<Partial<Armor>>({});
  const [itemObj, setItemObj] = useState<Partial<ItemType>>({});

  const [modalOpen, setModalOpen] = useState<boolean>(false);
  const [modalContent, setModalContent] = useState<ReactElement>(<></>);
  const [addButtonsStyle, setAddButtonsStyle] = useState<
    Record<ItemName, 'outlined' | 'contained'>
  >({
    weapon: 'contained',
    armor: 'outlined',
    item: 'outlined'
        ...
        <>
          ...
          <FormControl fullWidth>
            ...
            <Select
              labelId="rarity"
              id="raritySelect"
              value={weaponItem.rarity ?? 'Nenhuma'}
              label="Raridade"
              onChange={e => {
                setWeaponItem({});
              }}
        ...
        <>
          ...
          <FormControl fullWidth>
            ...
            <Select
              labelId="rarity"
              id="raritySelect"
              value={armorItem.rarity ?? 'Nenhuma'}
              label="Raridade"
              onChange={e => {
                setArmorItem(state => ({ ...state, rarity: e.target.value }));
              }}
            >
              <MenuItem value="Nenhuma">Nenhuma</MenuItem>
              <MenuItem value="Comum">Comum</MenuItem>
        ...
        <>
          ...
          <FormControl fullWidth>
            ...
            <Select
              labelId="rarity"
              id="raritySelect"
              value={itemObj.rarity ?? 'Nenhuma'}
              label="Raridade"
              onChange={e => {
                setItemObj(state => ({ ...state, rarity: e.target.value }));
              }}
            >
              <MenuItem value="Nenhuma">Nenhuma</MenuItem>
              <MenuItem value="Comum">Comum</MenuItem>
  ...
  const capacity = useMemo(() => {
    const weaponsWeight = f.values.inventory.weapons
      .map(weapon => weapon.weight)
      .reduce((a, b) => a + b, 0);
    const armorsWeight = f.values.inventory.armors
      .map(armor => armor.weight)
      .reduce((a, b) => a + b, 0);
    const itemsWeight = f.values.inventory.items
      .map(item => {
    ...
    const maxCargo = f.values.inventory.items
      .map(item => {
        if (item.kind === 'Capacidade') {
          return item.weight;
        } else return 0;
    ...
    return f.values.inventory.weapons.map(weapon => (
      <Grid item key={weapon.name}>
        <Item
          ...
          diceQuantity={
            Math.floor(f.values.attributes[weapon.hit] / 2 ?? 0) + 1
          }
          effect={{
  ...
  useEffect(() => {
    ...
    f.setFieldValue(
      'capacity.max',
      Number(Number(f.values.attributes.vig * 2 + maxCargo + 5)).toFixed(1)
    );
  }, [capacity]);
      ...
      <Box display="flex" flexDirection="column" gap={2}>
        <Box display="flex" alignItems="center" gap={2}>
          ...
          <Typography
            ...
            variant="h5"
            color={
              f.values.capacity.cargo / f.values.capacity.max >= 1
                ? red[500]
                : f.values.capacity.cargo / f.values.capacity.max >= 0.75
                  ? yellow[500]
                  : 'white'
            }
          >
