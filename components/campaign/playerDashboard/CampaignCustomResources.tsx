...
export default function CampaignCustomResources(): ReactElement | null {
        ...
        fetch(`/api/rpg-system/${charsheet.systemId}`)
            ...
            .then((data: RPGSystem) => {
                if (data.customResources && data.customResources.length > 0) {
                    ...
                    const current: Record<string, number> = { ...(charsheet.customResources ?? {}) };
                    let changed = false;
                    data.customResources.forEach(r => {
                        if (current[r.key] === undefined) {
                            current[r.key] = r.defaultValue ?? r.max;
                            changed = true;
                        }
                    });
                    if (changed) updateCharsheet({ customResources: current });
                }
            })
    ...
    const resources: Record<string, number> = charsheet.customResources ?? {};
